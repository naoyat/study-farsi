...
;              ("IN"     "i:n"      pron (こ))
              ...
              ("INJA"    "i:nja:"   pron  (ここ))
              ("U"       "u:"       pron* (彼 彼女)) ;性別を訳し分けたい

              ("JA"      "ja:"      n     (場所))
              ("TUT"     "tu:t"     n     (桑の実))
              ("HSIN"    "hosein"   prop  (ホセイン {人名} <male>))
              ...
              ("AQA"     "a:gha:"    n    (_氏 _さん <male>))
              ("KhANM"   "kha:nom"   n    (_夫人 _さん 婦人 <female>))
              ("KASB"    "ka:seb"    n    (商人))
              ("BNDR"    "bandar"    n    (_港))
              ("RUD"     "ru:d"      n    (_川))
              ("KUH"     "ku:h"      n    (_山))
              ...
              ("NADR"    "na:der"    prop (ナーデル {人名} <male>))
              ("HSN"     "hasan"     prop (ハサン {人名} <male>))
              ("@FShAR"  "afsha:r"   prop (アフシャール {人名}))
              ("RJBRzADH" "rajabza:de" prop (ラジャブザーデ {人名}))
              ("`BBAS"   "abba:s"    prop (アッバース {人名} <male>))
              ("`BAS"   "abba:s"     prop (アッバース {人名} <male>))
        ...
        (string-append
         (string-join
          (map (lambda (x) (case (pos x)
                             [(n pron pron*) (string-append (surface x) "の")]
                             [(prop) (if (#/^_/ body)
                                         (surface x)
...
(define (gender obj)
  (if (modified? obj)
      (gender (modified-body obj))
      (case (pos obj)
        [(n pron) (not (memq '<female> (cadr obj)))]
        [else #t])
      ))

(define (correspond-gender pron obj)
  (case (string->symbol pron)
    [(彼) (if (gender obj) "彼" "彼女")]
    [else pron]))
  ...
  (let* ([words (string-split text " ")]
    ...
    (let ([pos-l (map pos trans)]  ; (pron n be)
      ...
      (match pos-l
        ...
        [((or 'det 'pron 'pron*) 'qu 'be '?)
         (format "~aは~aですか？" (first ja) (second ja))]
        [((or 'n 'pron) (or 'n 'prop) 'be)
         (format "~aは~aです。" (first ja) (second ja))]
        [('pron* (or 'n 'prop) 'be)
         (format "~aは~aです。"
                 (correspond-gender (first ja) (second trans))
                 (second ja))]
        ;; lesson 7
        [((or 'n 'prop))
         (first ja)]
        [((or 'n 'pron 'pron*) 'adj 'be)
